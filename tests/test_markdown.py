...
def test_renders_successful_doc():
    ...
    assert "- [json.dumps](#json.dumps) — Serialize obj." in markdown
...
def test_renders_unresolved_doc():
    ...
    assert "## API reference" in markdown
    assert "- [missing](#missing) — Unresolved: Could not resolve missing." in markdown
...
def test_duplicate_anchors_get_disambiguated():
    ...
    assert "- [pkg.a](#pkg.a) — First." in markdown
    assert "- [pkg.A](#pkg.a-2) — Second." in markdown
